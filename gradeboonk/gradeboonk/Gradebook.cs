...
namespace gradeboonk
{
    class Gradebook
    {
        Section[] ClassHours = new Section[6];
        string curSection = "";

        //methods
        //returns -1 if section is not found, othewise returns the index for the section
        ...
        {
            //return false if section name exists
            int sectionIndex = getSectionIndexByName(sectionName);
            if ( sectionIndex != -1)
            {
               return false;
            }
            //return false if 6 sections already
            //add new section
            //change cur section 
            return false; //FIXME
        }
